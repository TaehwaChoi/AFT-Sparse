############################################
## Variable selection for smoothed AFT model
## 19 August, 2020
## By Taehwa Choi, Korea University
############################################
...
aft_pen =  function(y, z, delta, beta0, lambda, bandwidth = NULL, r = 3.7, type, max.iter = 100, eps = 1e-8)
{
   ...
   for (t in 1:max.iter)
   {
      ...
      for (j in 1:p)                                  # Step 7
      {
         zj = crossprod(z[,j], resid)/n + beta[j]     # Step 4
         
         new.beta[j] = if (type == "SCAD") {          # Step 5
         ...
         resid =  resid - z[,j] * (new.beta[j] - beta[j]) # Step 6
      }     
      # Step 8
